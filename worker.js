...
async function handleRequest(request) {
  ...
  if (request.method === 'GET') {
    if (path === "/" || path === "/services") return renderServicesPage()
    if (path === "/order") return renderOrderPage()
    if (path === "/status") return renderStatusPage()
    return notFound()
  }

  if (request.method === 'POST') {
    if (path === "/services") return handleServices()
    if (path === "/order") return handleOrder(request)
...
async function handleServices() {
  ...
  try {
    ...
    if (data.success) return jsonResponse(data)
    else return jsonResponse({ success: false, error: data.error || "Unknown error" }, 400)
  } catch(e) {
...
async function handleOrder(request) {
  ...
  if (!body.service || !body.target || !body.quantity) {
...
async function handleStatus(request) {
  ...
  if (!body.order_id) return jsonResponse({ success: false, error: "Missing order_id parameter" }, 400)
...
<style>
  * { box-sizing: border-box; }
  body {
    ...
    background: #f5f7fa; color: #222; min-height: 100vh; display: flex; flex-direction: column;
  }
  header {
    background: #004080; color: white; padding: 1rem 2rem; font-weight: 700; font-size: 1.5rem; letter-spacing: 0.05em;
    user-select: none;
  }
  main {
    flex: 1; max-width: 800px; margin: 2rem auto; background: white; border-radius: 10px; padding: 2rem;
    box-shadow: 0 0 20px rgb(0 0 0 / 0.1);
  }
  footer {
    text-align: center; padding: 1rem; font-size: 0.85rem; color: #666; user-select:none;
  }
  nav a {
    color: #eee; margin-right: 1rem; text-decoration: none; font-weight: 600;
  }
  nav a:hover, nav a.active {
    color: #ffcc00;
  }
  h1 {
    margin-bottom: 1rem; font-weight: 800; letter-spacing: 0.05em;
  }
  form label {
    display: block; font-weight: 600; margin-top: 1rem; margin-bottom: 0.3rem;
  }
  input[type=text], input[type=number], textarea, select {
    width: 100%; padding: 0.6rem 0.8rem; font-size: 1rem; border-radius: 6px; border: 1.5px solid #ccc;
  ...
  button {
    margin-top: 1.5rem; background-color: #004080; border: none; color: white;
    padding: 0.75rem 1.5rem; font-size: 1.1rem; font-weight: 700; border-radius: 8px; cursor: pointer;
  ...
  pre {
    background: #f0f0f0; border-radius: 8px; padding: 1rem; overflow-x: auto; margin-top: 1rem;
  }
  .response {
    margin-top: 1rem; background: #e0f7e9; border: 1px solid #00a152; padding: 1rem; border-radius: 8px;
    color: #006622; white-space: pre-wrap; word-wrap: break-word;
  }
  .error {
    background: #fdecea; border: 1px solid #e53935; color: #b71c1c;
  }
</style>
</head>
<body>
...
<footer>
  &copy; 2025 PT. JAGOAN PEDIA GROUP
</footer>
</body>
</html>`, {
...
function renderServicesPage() {
  return renderLayout("Services", `
    ...
    <p>Klik tombol di bawah untuk mengambil daftar layanan dari API JagoanPedia.</p>
      ...
      try {
        ...
        if (data.success) {
          if (Array.isArray(data.data)) {
            resultEl.textContent = data.data.map(s =>
              \`ID: \${s.id}\\nNama: \${s.name}\\nMinimal: \${s.min}\\nMaksimal: \${s.max}\\nHarga: \${s.price}\\nStatus: \${s.status}\\nCatatan: \${s.note}\\n---\`
            ).join('\\n')
          } else {
            const s = data.data
            resultEl.textContent = \`ID: \${s.id}\\nNama: \${s.name}\\nMinimal: \${s.min}\\nMaksimal: \${s.max}\\nHarga: \${s.price}\\nStatus: \${s.status}\\nCatatan: \${s.note}\`
          }
        } else {
...
function renderOrderPage() {
  return renderLayout("Order", `
    ...
    <form id="orderForm">
      ...
      <input type="text" id="service" name="service" placeholder="Misal: 1" required />
      ...
      <label for="username">Username Target (opsional, khusus like komentar IG)</label>
